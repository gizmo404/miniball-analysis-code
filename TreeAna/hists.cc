...
#define EBINS 500	// number of bins in electron spectra
#define EMAX 1000	// maximum energy in electron spectra
...
void hists::Initialise() {
	...
	for(int i=0; i<PHI_NSTEPS; i++){
		...
		//hname = "phical_" + doppler::convertInt(CLUSTER)+ "_dcT_MaxSigma";
		hname = "phical_dcT_MaxSigma";
		phical_dcT_MaxSigma = new TH2F(hname.c_str(),"Beam gated prompt, Target Doppler corrected #gamma-rays",PHI_NSTEPS, 0, PHI_NSTEPS,GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));
...
#endif
#ifdef CDCALIB
		hname = "GeCalib_cluCD_dcT_MaxSigma";
		GeCalib_clu_CD_MaxSigma = new TH2F(hname.c_str(),"Beam gated prompt, Target Doppler corrected #gamma-rays",8,0,7,CD_DIST_NSTEPS,0,CD_DIST_NSTEPS-1);
		for(int i=0; i<8; i++){
			for(int j=0; j<CD_DIST_NSTEPS; j++){
				hname = "GeCalib_clu_CD_dcT" + doppler::convertInt(i+1) +"CDDist"+doppler::convertInt(j);
				GeCalib_clu_CD[i][j] = new TH1F(hname.c_str(),"Beam gated prompt, Doppler corrected #gamma-rays VS CD dist",GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));
			}
		}
#endif

#ifdef DISTCALIB
		hname = "GeCalib_clu_dist_dcT_MaxSigma";
		GeCalib_clu_DIST_MaxSigma = new TH2F(hname.c_str(),"Beam gated prompt, Target Doppler corrected #gamma-rays",8,0,7,CLU_DIST_NSTEPS,0,CLU_DIST_NSTEPS-1);
		for(int j=0; j<CLU_DIST_NSTEPS; j++){
			hname = "GeCalib_clu_dist_dcT" + doppler::convertInt(CLUSTER) +"CluDist"+doppler::convertInt(j);
			GeCalib_clu_DIST[j] = new TH1F(hname.c_str(),"Beam gated prompt, Doppler corrected #gamma-rays VS CD dist",GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));
		}
#endif
	 
#ifdef ALPHACALIB
		hname = "GeCalib_clu_alpha_dcT_MaxSigma";
		GeCalib_clu_ALPHA_MaxSigma = new TH2F(hname.c_str(),"Beam gated prompt, Target Doppler corrected #gamma-rays",8,0,7,CLU_ALPHA_NSTEPS,0,CLU_ALPHA_NSTEPS-1);
		for(int j=0; j<CLU_ALPHA_NSTEPS; j++){
			hname = "GeCalib_clu_alpha_dcT" + doppler::convertInt(CLUSTER) +"CluAlpha"+doppler::convertInt(j);
			GeCalib_clu_ALPHA[j] = new TH1F(hname.c_str(),"Beam gated prompt, Doppler corrected #gamma-rays VS detector alpha",GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));
		}
#endif

#ifdef SPEDECALIB
		hname = "SpedeCalib_dist_mat_dcT";
		SpedeCalib_dist_mat = new TH2F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",SPEDE_DIST_NSTEPS,0, SPEDE_DIST_NSTEPS-1,EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));

		for (int j=0; j<SPEDE_DIST_NSTEPS; j++){
			hname = "SpedeCalib_dist_dcT_" + doppler::convertInt(j);
			SpedeCalib_dist[j] = new TH1F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));
		}
		hname = "SpedeCalib_alpha_mat_dcT";
		SpedeCalib_alpha_mat = new TH2F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",SPEDE_DIST_NSTEPS,0, SPEDE_DIST_NSTEPS-1,EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));

		for (int j=0; j<SPEDE_DIST_NSTEPS; j++){
			hname = "SpedeCalib_alpha_dcT_" + doppler::convertInt(j);
			SpedeCalib_alpha[j] = new TH1F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));
		}
		hname = "sid_test";
		sid_test = new TH1F(hname.c_str(),"SIS TESTS",50,0,50);
		for (int j=0; j<50; j++){
			hname = "sid_single_test" + doppler::convertInt(j);
			sid_single_test[j] = new TH1F(hname.c_str(),"sid_single_test",EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));
			hname = "sid_dist_" + doppler::convertInt(j);
			sid_single_dist[j] = new TH2F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",SPEDE_DIST_NSTEPS,0, SPEDE_DIST_NSTEPS-1,EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));
			hname = "sid_alpha_" + doppler::convertInt(j);
			sid_single_alpha[j] = new TH2F(hname.c_str(),"Beam gated prompt, Doppler corrected electrons",SPEDE_DIST_NSTEPS,0, SPEDE_DIST_NSTEPS-1,EBINS,-0.5*((float)EMAX/(float)EBINS),EMAX-0.5*((float)EMAX/(float)EBINS));
		}
#endif
	/* Backgorund subtracted, Doppler corrected spectra */
...
void hists::Fill2h( double GEn, double GTh, double GPh, vector <double> GCor_GEn, vector <double> GCor_GTh,
					vector <double> GCor_GPh, vector <int> GCor_CluID, vector <double> GCor_Gtd, bool electron,
					double *PEn, Int_t *Pann, Int_t *Psec, Int_t *Pquad, Int_t *Pptr, double *td, double weight ) {
...
void hists::FillGam2h( double GEn, double GTh, double GPh, double *PEn, Int_t *Pann,
						Int_t *Psec, Int_t *Pquad, Int_t Bptr, Int_t Tptr, double weight ) {
...
void hists::FillElec2h( double GEn, double GTh, double GPh, double *PEn, Int_t *Pann,
						Int_t *Psec, Int_t *Pquad, Int_t Bptr, Int_t Tptr, double weight ) {
...
void hists::FillGamGam2h( double GEn, double GTh, double GPh, vector <double> GCor_GEn, vector <double> GCor_GTh,
						 vector <double> GCor_GPh, vector <int> GCor_CluID, vector <double> GCor_Gtd, double *PEn,
						 Int_t *Pann, Int_t *Psec, Int_t *Pquad, Int_t Bptr, Int_t Tptr, double weight ) {
...
void hists::PhiCalHists( double GEn, double GTh, double GPh, double PEn, Int_t Pann,
	...
	for( int j = 0; j < PHI_NSTEPS; j++ ) {
		...
		phical_dcT_MaxSigma->Fill(j, GEn*doppler::DC(BEn,BTh,BPh,GTh,GPh,AP));
		//cout << "j " << j << " energy  " << GEn*doppler::DC(BEn,BTh,BPh,GTh,GPh,AP) << "\n";
	}
...
void hists::FillPar1h( double PEn, Int_t Pann, Int_t Psec, Int_t Pquad, Int_t cut, double weight ) {
	...
	partQ[Pquad]->Fill(PTh, PEn/1000., weight);
...
void hists::FillPar2h( double *PEn, Int_t *Pann, Int_t *Psec, Int_t *Pquad, Int_t Bptr, Int_t Tptr, double weight ) {
	...
	partQ[Bquad]->Fill(BTh, BEn/1000., weight);
	partQ[Tquad]->Fill(TTh, TEn/1000., weight);
