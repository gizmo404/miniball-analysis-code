...
Int_t doppler::Cut(Double_t PEn, Double_t anno, Int_t quad) {
	...
	/* This is disabling the cuts
	else identity = 1; // no gate
	//*/
	...
	// normal kinematics with Beam/Target separation (change the else above to be false)
		...
		double a = 693.8, b = -13.6165, c = 0.0404618;
		double ang = GetPTh(anno) * TMath::RadToDeg();

		//cout << "PEn/1000 " << PEn/1000 << " " << a + b*ang + c*ang*ang << "\n";

		if( PEn/1000. >= ( a + b*ang + c*ang*ang ) )

			identity = 1; // beam
			
		//else if( PEn/1000. < ( a + b*ang + c*ang*ang ) && PEn > 100. )
		else if( PEn/1000. < ( a + b*ang + c*ang*ang ) && PEn > 100. )
...
//Setup to take the CD distance as an input to help with calibrating
Double_t doppler::GetPTh(Double_t anno, Double_t Dist) {
	/* Returns theta angle from ann strip number in radians */
	return TMath::ATan((9.+(15.5-anno)*2.)/Dist);
}
